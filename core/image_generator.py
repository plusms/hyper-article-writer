...
def analyze_reference_images(
    ...
    prompt = (
        ...
        "【重要】何が描かれているか（人物・物・場面）ではなく、どのように描かれているか（線質・塗り・色・余白）のスタイル情報のみ記述すること。"
        "カラー系フィールドは画像から読み取れる支配色をHEX形式（例: #3B82F6）で出力すること。"
        "AI画像生成プロンプトとして直接使える日本語・具体的な記述にすること。"
        "JSONのみ出力（コードブロック・説明文不要）。" + color_hint + """

{
  "primary_color": "画像の支配的なメインカラー（HEX形式。例: #3B82F6）",
  "accent_color": "アクセント・強調色（HEX形式。例: #F59E0B）",
  "background_color": "背景色（HEX形式。例: #FFFFFF）",
  "text_color": "テキスト・文字色（HEX形式。例: #333333）",
  "illustration_style": "イラストの様式（例: フラットデザイン・手描き風・アイコン系など）",
  "line_weight": "線の太さ・質感（例: 均一な細線2px・太めのアウトライン・線なしなど）",
  "character_style": "人物・キャラクターを描く場合のスタイル（例: 4頭身・記号的表現・写実的など）。人物が必要かどうかは画像ごとに異なるため、ここには描き方のみ記述する",
  ...
  "additional_notes": "その他の特徴（例: 全体的にクリーン・医療系の清潔感など）",
  "ref_image_analysis": "ビジュアルスタイルの総合説明（200字以内）。何を描くかは含めず、線・塗り・色調・余白・雰囲気のみ記述する"
}"""
    )
