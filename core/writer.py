...
Q5. その他NGワード・NGパターンが使われていないか（傾向・設計・前提・把握・整理・方向性・もちろん・大切です・なお・順番に・動線・糸抜き・現実的な選択肢・後悔しにくい・失敗しにくい・スムーズ・活用・言えます・言えるでしょう・「判断につながります」「判断しやすくなります」等の間接的判断表現 ／ 「多くの方が迷うのが〜」「〜と迷う方も少なくありません」「〜というケースが多く、」「広告料金」）
...
def _build_body_prompt(
    ...
    site_parts_block = (
        ...
        "- 「箇条書き（リスト）」「箇条書き（チェックリスト）」「箇条書き（数字）」パーツ → 対応するリストに使用\n"
...
- 段落: <p> タグ（裸のテキスト禁止）
- マーカー: <span class="marker">テキスト</span>（各H3に1か所のみ。そのH3で最も伝えたい主張の核心フレーズを囲む。意味が伝わる最小範囲にする。一文まるごとを囲まない。単語1語のみを囲まない）
- 太文字: <span class="b">テキスト</span>（各H3に1〜2か所。サブ的な主張・強調したい語句を囲む）
