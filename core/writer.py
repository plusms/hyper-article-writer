...
_AFFILI_LINK_RE = re.compile(r"^送客リンク：\s*(\S+)\s*$", re.M)


def extract_affiliate_links(clinic_info: dict) -> dict:
    """案件情報のテキストから、案件ごとの送客リンクを取り出す。"""
    links = {}
    for name, info in (clinic_info or {}).items():
        match = _AFFILI_LINK_RE.search(info or "")
        if match:
...
■ 絶対NGワード
- 抽象語：傾向・設計・前提・把握・整理・方向性・実態・重要
- 結論をぼかす：現実的な選択肢になります・後悔しにくい・失敗しにくい・納得感のある
- テンプレ語：もちろん・大切です・なお・順番に・動線・生活動線・糸抜き（→抜糸）
- 不自然な強調：救世主・味方・第一歩・鍵・近道・スムーズ・最適・最もふさわしい・活用
- 誰目線か不明：「実態」「現状」「現場では」

■ 提携申請で修正依頼が来る表記（使わない）
- 「上記」「下記」→ 記事内の場所を指さず、内容そのものを書く
- 「安全性」→「品質性」
- 「購入」→「入手」
- 「オンライン完結」「オンラインで完結」→ 対面診療との組み合わせが基本のため完結表現は使わない。「オンラインで手続きができる」と書く
- 「電話のみ」「電話だけで診療」→ 音声と映像による診療が原則のため、電話だけで診療できる表現は使わない
- 数値・医学的主張を書いた箇所には出典元URLを添える
...
- 「広告料金」
- 「〜することで判断できる/把握できる/整理できる/確認できる」等、認識系動詞で結ぶ構文（ユーザーは理解したいのではなく行動したいので、「〜すると選べます」「〜により申し込めます」等の行動・状態変化に言い換える）
- 「以下のとおり」

■ 括弧・引用符ルール
- 文中の強調・固有名詞表記に「」（隅付き括弧）を使わない。強調は<strong>タグで表現する
- （）（丸括弧）は文意が変わらないと困る補足（例：シアリス（タダラフィル）、副作用（かゆみ・腫れ）等）のみ使用する。説明的な内容を（）で括るのは禁止
- ""（ダブルクォーテーション）をHTML内の引用符として使わない（class=""marker""のようにHTMLの属性・テキスト中で二重引用符になる原因になる）
...
- まとめのH2にはH3を入れない（<p>・テーブル・箇条書きのみで構成する）
...
Q29. 各H3に<span class="marker">が1つ入っているか（そのH3で最も伝えたい核心フレーズ。数値・条件・判断基準が入っていることが多い。一文まるごとはNG。囲む範囲が30字を超えているもの・マーカー内に句点が入っているものもNG）
...
def _build_body_prompt(
    ...
    # 登録されているパーツだけを「使え」と言う。format_site_parts は "■ 名前" の形で並べる。
    # 空のスロットまで使えと書くと、存在しないものを使うことになり、モデルが
    # h2-title のようなクラス名を発明する。無いものは見本HTMLに倣わせる。
    ...
    site_parts_block = (
        f"\n{site_parts}\n"
        "【HTMLの書き方の順番（厳守）】\n"
        "1. 上のパーツ一覧にあるものは、そのHTMLをそのまま使う\n"
        "2. 一覧に無いものは見本HTMLと同じ形で書く。見本が裸の<h2>なら裸の<h2>を使う\n"
        "3. どちらにも無いクラス名を新しく作らない\n"
        "【一覧にあるパーツの使いどころ】\n"
        + ("- 「H2」パーツ → すべてのH2見出しに使用（裸の<h2>タグ禁止）\n" if _has_part("H2") else "")
        + ("- 「H3」パーツ → すべてのH3見出しに使用（裸の<h3>タグ禁止）\n" if _has_part("H3") else "")
        + ("- 「小見出し」パーツ → テーブル・箇条書きの直前に必ず使用（<p>や<strong>で代替しない）\n" if _has_part("小見出し") else "")
        + ("- 「箇条書き」系パーツ → 対応するリストに使用（裸の<ul><li>タグ禁止）\n" if _has_part("箇条書き（リスト）") else "")
        + ("- 「ボックス」パーツ → 箇条書き全体をこのパーツで包む\n" if _has_part("ボックス①（枠のみ、背景色なし）") else "")
        + ("- 「マーカー」パーツ → 各H3で最も伝えたいフレーズを1か所のみ。囲むのは30字以内。マーカー内に句点を入れない（文末の「〜ます。」まで囲まない）\n" if _has_part("マーカー") else "")
        + ("- 「太文字」パーツ → 各H3でサブ主張・強調箇所を1〜2か所\n" if _has_part("太文字") else "")
        + "- {{変数名}}は実際の内容に置き換えること\n"
    ) if site_parts else ""

    # HTML出力ルール：サイトパーツがある場合はパーツ参照に切り替え（汎用タグ指定と矛盾しないよう）
    if site_parts:
        if _has_part("H2") and _has_part("H3"):
            _html_heading = "- 見出し(H1): <h1> タグを使用\n- 見出し(H2/H3): 「H2」「H3」パーツのHTML形式を使用（裸の<h2><h3>タグ禁止）"
        else:
            _html_heading = "- 見出し(H1): <h1> タグを使用\n- 見出し(H2/H3): パーツ一覧に無いので見本HTMLと同じ形で書く。見本が裸の<h2><h3>タグなら裸のタグを使う"
        if _has_part("マーカー"):
            _html_marker = '- マーカー: 「マーカー」パーツのHTML形式を使用。各H3に1か所のみ。核心フレーズのみ囲む（一文まるごと・単語1語だけは禁止）。囲むのは30字以内。マーカー内に句点を入れない（文末の「〜ます。」まで囲まない）'
        else:
            _html_marker = '- マーカー: パーツ一覧に無いので見本HTMLと同じ形で書く。各H3に1か所のみ。囲むのは30字以内。マーカー内に句点を入れない'
        if _has_part("太文字"):
            _html_bold = '- 太文字: 「太文字」パーツのHTML形式を使用。各H3に1〜2か所。強調したい語句を囲む'
        else:
            _html_bold = '- 太文字: パーツ一覧に無いので見本HTMLと同じ形で書く。各H3に1〜2か所'
    else:
        _html_heading = "- 見出し: <h1>、<h2>、<h3> タグを使用（<h4>は禁止）"
        _html_marker  = '- マーカー: <span class="marker">テキスト</span>（各H3に1か所のみ。そのH3で最も伝えたい主張の核心フレーズを囲む。意味が伝わる最小範囲にする。一文まるごとを囲まない。単語1語のみを囲まない。囲むのは30字以内。マーカー内に句点を入れない＝文末の「〜ます。」まで囲まない）'
...
【使ってよい事実の範囲（最優先・クリニック以外にも適用）】
- 上のクリニック情報と、この指示に書かれていることだけを事実として書く。書かれて
  いないことは書かない。禁止する項目を数え上げているのではない。書いてよい範囲を
  ここに限定している
- 対象は料金・院の情報だけではない。効果が出るまでの回数と期間、施術の間隔、痛みの
  程度、機器の仕組み、地域の気候・混雑時期・人口・交通、法令や制度、統計、いずれも
  上のデータに無ければ書かない
- 一般論としてどうしても必要な場合は、断定せずに `[要確認：〜]` の形で残す。数字を
  自分で決めて書かない
- 読者に向けた判断・比較・言い換えは自由に書いてよい。制限がかかるのは事実の記述だけ

【表のセルに要確認を書かない（厳守）】
- テーブルのセルにその案件の情報が無い場合は「−」を入れる。[要確認]と書かない
- セルは空欄か「−」で読者に意味が通る。[要確認]が残ったまま公開されると事故になる
- [要確認]を使ってよいのは本文の文章の中だけ

【送客リンクが無い案件（厳守）】
- 4位以降の案件にはリンクもCTAボタンも置かない。そういう仕様である
- リンクが無いことを [要確認] に書かない。人に確認を求める必要はない
...
{_html_marker}
{_html_bold}
- 比較表: パーツ一覧の「テーブル」「スクロールテーブル」があればそれを使う。無ければ見本HTMLと同じ形にする。クラス名を勝手に付けない
...
- `<!-- H2_BLOCK_START:... -->` と `<!-- クリニック紹介ブロック入る -->` 以外のHTMLコメントは出力しない
{todo_note}
...
⑦ H4タグが使われていないか
⑦' 「〜することで判断/把握/整理/確認できる」等の認識系で終わる文がないか（「〜すると選べます」「〜により申し込めます」等の行動・状態変化に置き換える）
⑦'' ""（ダブルクォーテーション）がHTMLの属性値・テキスト内で引用符として使われていないか（class=""marker""等の二重引用符になっていないか）

■ 見出しチェック
⑧ H2・H3に「（）」「｜」「、」「【】」「徹底」が使われていないか
⑨ メインKW「{inputs['main_kw']}」が今回生成するH2のいずれかに含まれているか。含まれていなければ修正する
⑩ サブKW（{', '.join(inputs['sub_kw'])}）それぞれが最低1つのH2に含まれているか。未含有のサブKWがあれば修正する
⑩' H2見出しのニュアンスが他のH2と重複していないか（似た意図のH2は統合する）
⑩'' まとめのH2にH3が含まれていないか（H3がある場合は<p>に展開する）
...
■ タグ・マークアップチェック
⑲ 各H3に<span class="marker">が1つ入っているか。入っていないH3があれば核心フレーズ（数値・条件・判断基準）を選んでmarkerを追加する。囲む範囲が30字を超えているもの・マーカー内に句点が入っているものは、核心フレーズだけに縮める
