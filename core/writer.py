...
② 各H3の末尾がSoWhat（読者が次のタッチポイントで踏む具体的な行動まで書いた締め文）になっているか
   NG：「〜重要です」「〜大切です」「〜しましょう」「〜確認してください」「〜できます」で行動が曖昧なまま終わる
   OK：「カウンセリングで〜を確認することで、契約前に費用の見通しを立てられます」「初診時に〜を伝えることで、医師が最適なプランを提示しやすくなります」等・読者が次に踏む場面が具体的に見える
...
⑪ 各H3に<span class="marker">が1つ入っているか。入っていないH3があれば、そのH3で最も伝えたい核心フレーズ（数値・条件・判断基準など）を選んでmarkerを追加する
