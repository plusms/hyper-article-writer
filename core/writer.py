...
- 各H3の末尾は必ずSoWhat。「〜ということがわかります」「〜になります」で終わらない
- H3の段落数は3〜5段落（<p>1つ・<table>1つ・<ul>/<ol>1つをそれぞれ1段落としてカウント）
- H3内にテキスト<p>を最低1つ入れる。テーブル・箇条書きのみはNG
- 冒頭文はAFDE構成：A（読者の悩みを具体的に言語化）F（誰向けか絞り込む）D（記事で分かることを端的に示す）E（信頼性の根拠）
- H2直下に1〜2文の導入文を入れる。H3・テーブル・リストで直接始めない
...
- H2・H3はそのKW・ジャンル・地域でしか通じない固有の表現にする
...
Q5. その他NGワードが使われていないか（傾向・設計・前提・把握・整理・方向性・もちろん・大切です・なお・順番に・動線・糸抜き・現実的な選択肢・後悔しにくい・失敗しにくい）
...
■ 構成・ブロックルール
Q13. H2直下（最初のH3の前）に導入文（1〜2文）があるか。いきなりH3・テーブル・リストで始まっていないか
...
Q20. 各H3の末尾にSoWhat（読者の判断・選択・行動につながる締め文）が入っているか
...
_TYPE_INSTRUCTIONS = {
    "地域": """
【地域記事 執筆要件（厳守）】
- 各H3本文に「地域名を別地域に置き換えると成立しない文」を最低1段落（3文以上）入れる（地域固有情報）
- 入れられない場合、そのH3は削除するか、別の地域固有トピックに差し替える（無理やり地名を挿入しない）
- 「どの地域でも同じことが言える一般論」に無理やり地名を入れて地域記事に見せかけない
- ノウハウ・メカニズム解説（「〇〇とは」「なぜ〇〇になるか」等）は最小限にとどめる。ノウハウ記事との競合（カニバリ）を避けるため、地域・クリニック選びに直結しない説明は省く
