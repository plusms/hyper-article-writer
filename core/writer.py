...
def _build_body_prompt(
    ...
    site_parts_block = (
        ...
        "- 「マーカー」パーツ → 各H3で最も伝えたいフレーズを1か所のみ。サイトパーツのHTML形式を使う（<span class=\"marker\">等の汎用タグ禁止）。囲むのは30字以内。マーカー内に句点を入れない（文末の「〜ます。」まで囲まない）\n"
    ...
    if site_parts:
        ...
        _html_marker  = '- マーカー: 「マーカー」サイトパーツのHTML形式を使用（<span class="marker">等の汎用タグ禁止）。各H3に1か所のみ。核心フレーズのみ囲む（一文まるごと・単語1語だけは禁止）。囲むのは30字以内。マーカー内に句点を入れない（文末の「〜ます。」まで囲まない）'
...
【見出しとブロックのHTML（厳守）】
- 見出し・小見出し・ボックス・テーブルのHTMLは見本と同じ形にする
- 見本に無いクラス名を作らない。h2-title・h2-ttl のような独自の名前を発明しない
- 見本が生の h2 タグを使っていれば生の h2 タグを使う。div に置き換えない
