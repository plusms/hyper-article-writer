...
_CLINIC_FIELDS = """\
院名：
院別住所・診療時間（見つかった院のみ。形式：院名 / 住所（番地まで） / 診療時間 を1院1行で列挙）：
休診日：
予約方法：
...
診察方法（対面・オンライン・電話）：
途中解約（解約条件・違約金等）：
参照URL（情報を取得したページのURL。複数ある場合はカンマ区切りで列挙。毎回必ず記録すること）：
全国院数：
その他の特徴・強み（上記フィールド以外でサイトから読み取れた固有情報を自由記述。見つからない場合は空欄）："""
...
- 【追加指定ページ情報】セクションの内容も「■ 基本項目」の各フィールドに反映すること。特に住所・診療時間・料金等のページ固有情報は積極的に使うこと
- 「■ 追加項目」：基本項目以外で記事執筆に使える情報があれば自由に追加してよい（なければ「なし」）
- 「■ LP情報」：コンテンツ内の【LP情報（ランディングページ）】セクションから読み取れた情報のみ転記する（なければ「なし」）
