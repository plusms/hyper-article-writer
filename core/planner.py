...
def generate_structure(inputs: dict, competitor_analysis: dict, clinic_info: dict, claude_api_key: str, gemini_api_key: str = "", article_provider: str = "claude", base_structure: str = "") -> dict:
    ...
    if article_type == "地域":
        ...
        type_note = (
            ...
            "【必須H2（省略不可・構成の末尾に必ず配置）】\n"
            "- CV記事誘導H2：記事末尾から2番目に配置。読者の次のアクション（クリニック選び・費用確認）への橋渡し\n"
            "- まとめH2：記事の最後に必ず配置。「まとめ」という名称は使わない。記事内容を表す見出しにする\n"
    ...
    _sparse_list = (len(inputs["clinics"]) <= 1 and _clinic_count == 0 and article_type not in ("商標",))
    if _sparse_list:
        _list_rule = (
            "以下のリストのクリニックを必ず含め、競合の掲載院数に合わせてリスト外のクリニックも"
            "「[要確認]クリニック名」として追加してよい（H3に[要確認]と表記）。"
        )
    else:
        _list_rule = (
...
- 【地域・比較記事限定】H2見出しにクリニック名・ブランド名を含めない。クリニック名はH3（クリニック紹介ブロック内）または本文に記載すること

■ H2構成
- H2の合計数は6〜8本を目安とする（固定H2を含む）。不要なH2を増やさない
...
- **H2見出し文言のニュアンスが重複しないこと。「〜の選び方」と「〜を選ぶポイント」のように似た意図の見出しを並べない**
...
■ H3構成
- 各H2に2〜4本（おすすめクリニック紹介H2・まとめH2の直下にはH3を置かない）
...
【タイトル作成ルール】
- 30文字前後（検索結果で途切れにくい目安）
- メインKWを先頭に含める
- サブKWを2つとも自然に含める
- 具体的・定量的な表現を入れる（例：「料金比較」「ポイント3選」「○○解説」）
- 記号は「！」のみ使用可。補足の（）はOKだがそれ以外の括弧は使用不可
- 競合タイトルに含まれているKWをタイトルに取り込む（検索意図が近いKWの可能性があるため）
- タイトルと導入文・記事内容の整合性を確認してから出力する

メタディスクリプション: （下記ルール厳守）

【メタディスクリプション作成ルール】
- 80〜100文字
- 前半にメインKW・サブKWを自然に含める（記事のテーマ提示）
- 後半に補足情報（記事で解決できる問題・読者が得られるメリット）を入れる
- 記事の具体的な内容（料金・院名・数字）には踏み込まない
- 過剰なキーワード詰め込みNG
...
⑧ 【ノウハウ記事のみ】H2リストの末尾2つが「CV記事誘導H2」→「まとめH2」の順になっているか。なければ追加する
⑨ タイトル案①がメインKWを先頭に含み、サブKWを2つとも含んでいるか。含まれていなければ修正する
⑩ タイトル案①が30文字前後か。大幅に超えている場合は短縮する
⑪ メタディスクリプションが80〜100文字か。前半にKW、後半に補足情報の構造になっているか
"""
