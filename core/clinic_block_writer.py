...
def generate_lower_blocks(entries: list, main_kw: str = '', sub_kw: list | None = None,
    ...
    prompt = (
        ...
        + '- 見出しのHTMLは見本と同じ形にする。見本に無いクラス名を作らない' + NL
        + '- 見出しは上位院の紹介と同じ階層にする。記事全体の大見出しと同じ階層にしない' + NL
        + '- リンク・CTAボタン・画像・料金テーブル・基本情報テーブルを出さない' + NL
        + '- リンクが無いことを要確認として書かない。4位以降はリンクを置かない仕様である' + NL
        ...
        + '- 数字を自分で決めて書かない' + NL        + '- 各院の情報に書いてあることは、書いてある事実として扱う。提供されていない、'
        + '  DBに記載なし、別途確認が必要、といった注記を付けない' + NL        + '- 要確認と書いてよいのは、記事に必要なのに各院の情報に本当に無い項目だけ' + NL        + '- リンクが無いこと、CTAが無いことを要確認に書かない' + NL + NL
        + '【共通ルール】' + NL
